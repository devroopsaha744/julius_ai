...
export class DeepgramSTTService {
      ...
      return await new Promise<boolean>((resolve) => {
        try {
          ...
          const onOpen = () => {
            ...
            try { this.keepaliveTimer = startKeepalive(ws as any, 12000); } catch {}
            cleanup();
            resolve(true);
          };
          const onError = () => {
            try { if (this.errorCallback) this.errorCallback('Deepgram connect failed; will retry on next audio chunk'); } catch {}
            cleanup();
            resolve(false);
          };
  ...
  private _setupListeners() {
        ...
        if (parsed && parsed.channel && Array.isArray(parsed.channel.alternatives)) {
          ...
          if (parsed.speech_final === true && transcript.trim()) {
            ...
            if (this.utteranceEndCallback) {
              try { this.utteranceEndCallback(parsed); } catch {}
            }
            return;
          }
        }
    ...
    this.ws.on('error', (err) => {
      try { if (this.errorCallback) this.errorCallback(`Deepgram stream error: ${String(err)}`); } catch {}
    });
    this.ws.on('close', () => {
      this.ws = null;
      try { if (this.keepaliveTimer) stopKeepalive(this.keepaliveTimer); } catch {}
  ...
  async sendAudio(audioData: Buffer | ArrayBuffer | string) {
    ...
    try {
      ...
      try {
        const ok = await this.connect();
        if (ok && this.ws) { try { (this.ws as any).send(audioData as any); } catch {} }
      } catch {}
    }
  }
